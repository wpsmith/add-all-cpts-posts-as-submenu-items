...
class Add_All_CPTPosts_As_Submenu_Items {
	 ...
	 * @var string
	 */
	public $form_idtracker       = 'aacptpasi-idtracker';
	 ...
	 * @var string
	 */
	public $form_checkbox_prefix = 'aacptpasi-child-checkbox-';
	 ...
	 * @var string
	 */
	public $ajax_action          = 'aacptpasi_checkbox_helper';
	 ...
	 * @var string
	 */
	public $meta_key             = '_aacptpasi_enabled';
	...
	function __construct() {
		if ( is_admin() ) {
			load_plugin_textdomain( 'add-all-cpt-posts-as-submenu-items', false, dirname( plugin_basename( __FILE__ ) ) . '/localization/' );
	...
	public function maybe_enqueue_script( $hook_suffix ) {
		...
		$script_slug = 'aacptpasi-checkboxes';
		...
		$params = array(
			...
			'checkboxdesc'       => __( 'Automatically add all CPT posts as submenu items', 'add-all-cpt-posts-as-submenu-items' ),
			'checkboxdesccustom' => __( 'Automatically add all CPT posts as submenu items. Input CPT registration name into the label (e.g., page, post, nav_menu_item).', 'add-all-cpt-posts-as-submenu-items' ),
		);

		wp_localize_script( $script_slug, 'aacptpasiParams', $params );
	}
	...
	public function add_children_to_menu( $items ) {
		...
		foreach ( $items as $item ) {
			...
			$items[ $i ] = $this->fix_top_menu_item( $item );
		}

		return $items;
	}
	
	public function fix_top_menu_item( $item ) {
		$obj = get_post_type_object( $item->xfn );
		//$item->title = $obj->label;
		$item->url = isset( $obj->has_archive ) && $obj->has_archive ? get_post_type_archive_link( $obj->name ) : '#';
		return $item;
	}
...
$add_descendants_as_submenu_items = new Add_All_CPTPosts_As_Submenu_Items();
